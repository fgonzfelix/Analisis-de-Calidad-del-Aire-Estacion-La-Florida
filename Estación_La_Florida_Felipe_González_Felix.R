...
#Cargamos las librerias utilizadas en el proyecto:
...
#Verificamos el tipo de datos de nuestras variables de Temperatura y Humedad:
typeof(df_Humedad$HORA..HHMM.)
typeof(df_Temperatura$HORA..HHMM.)

#Preprocessing
#Convertimos los datos de Temperatura horaria a diaria:

#Convertimos las columnas de fecha y hora al formato adecuado
...
#Agrupamos los datos por día y calcular el promedio de la columna "X" para cada día
df_Temperatura <- df_Temperatura %>%
  group_by(FECHA..YYMMDD) %>%
  summarize(X = mean(X))

#Verificamos la cantidad de filas resultantes
nrow(df_Temperatura)
...
#Cambiamos el nombre de la columna "X.1" por Temp
names(df)[10] <- "Temp"
...
#Por ultimo realizamos una predicción con Arima de nuestra variable que mas afecta el PCA_1, o sea la Temperatura.
#Primero visualizamos rapidamente nuestros datos temporales diarios de la temperatura:
